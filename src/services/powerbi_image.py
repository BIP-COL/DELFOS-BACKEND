...
async def list_powerbi_pages(
    ...
    Returns:
        list: List of page names or None if error
    """
    if aiohttp is None:
        return None
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(pages_url, headers=headers) as response:
                ...
                if "value" in result:
                    pages = [page.get("name") for page in result["value"]]
                    logger.info(f"Found {len(pages)} pages in report: {pages}")
                    return pages
                else:
...
async def export_powerbi_page(
    ...
    export_url = (
        f"{POWER_BI_API_BASE}/groups/{workspace_id}/"
        f"reports/{report_id}/pages/{page_name}/ExportTo"
    )
...
async def get_powerbi_image(run_id: str, tipo_grafico: str) -> Optional[bytes]:
    ...
    try:
        # Step 1: Map tipo_grafico to Power BI page name
        page_name = VISUAL_PAGE_MAP.get(tipo_grafico.lower())
        if not page_name:
            logger.warning(f"Unknown tipo_grafico: {tipo_grafico}. Valid values: {list(VISUAL_PAGE_MAP.keys())}")
            return None
        
        # Step 2: Get Power BI access token
        logger.debug(f"Getting Power BI access token for run_id: {run_id}")
        access_token = await get_powerbi_access_token()
        
        # Step 3: Export the page as image
